...
BOOL CSystemTray::ClearCheckedItems()
{
...
BOOL CSystemTray::ClearGrayedItems()
{
	m_GrayedItems.clear();
...
BOOL CSystemTray::ClearGrayedPositions()
{
	m_GrayedPositions.clear();

	return TRUE;
}

BOOL CSystemTray::SetCheckedItems(HMENU hMenu)
{
	for (int i = 0; i < m_CheckedItems.size(); i++)
		::CheckMenuItem(hMenu, m_CheckedItems[i], MF_CHECKED | MF_BYCOMMAND);

	return TRUE;
}

BOOL CSystemTray::SetGrayedItems(HMENU hMenu)
{
	for (int i = 0; i < m_GrayedItems.size(); i++)
		 ::EnableMenuItem(hMenu, m_GrayedItems[i], MF_GRAYED | MF_BYCOMMAND);

	return TRUE;
}

BOOL CSystemTray::SetGrayedPositions(HMENU hMenu)
{
	for (int i = 0; i < m_GrayedPositions.size(); i++)
		 ::EnableMenuItem(hMenu, m_GrayedPositions[i], MF_GRAYED | MF_BYPOSITION);
...
BOOL CSystemTray::CheckMenuItem(UINT uiCmd, BOOL bCheck)
{
	for (int i = 0; i < m_CheckedItems.size(); i++)
	{
		if(m_CheckedItems[i] == uiCmd)
		{
			m_CheckedItems.erase(m_CheckedItems.begin() + i);
			break;
		}
	}
	...
	if(bCheck)
		m_CheckedItems.push_back(uiCmd);

    return TRUE;
}

BOOL CSystemTray::GrayMenuItem(UINT uiCmd, BOOL bGray)
{
	for (int i = 0; i < m_GrayedItems.size(); i++)
	{
		if(m_GrayedItems[i] == uiCmd)
		{
			m_GrayedItems.erase(m_GrayedItems.begin() + i);
			break;
		}
	}

	if(bGray)
		m_GrayedItems.push_back(uiCmd);

    return TRUE;
}

BOOL CSystemTray::GrayMenuPosition(UINT uiPos, BOOL bGray)
{
	for (int i = 0; i < m_GrayedPositions.size(); i++)
	{
		if(m_GrayedPositions[i] == uiPos)
		{
			m_GrayedPositions.erase(m_GrayedPositions.begin() + i);
			break;
		}
	}

	if(bGray)
		m_GrayedPositions.push_back(uiPos);
...
LRESULT CSystemTray::OnTrayNotification(UINT wParam, LONG lParam) 
{
    ...
    {    
		...
		SetCheckedItems(hSubMenu);
		SetGrayedItems(hSubMenu);
		SetGrayedPositions(hSubMenu);
